...
**毕业论文自动排版系统** 是一个基于规则配置驱动的学术文档排版工具，通过可视化规则配置 + 自动排版引擎，快速生成符合学校规范的论文。内置三线表、ER 图、系统架构图、流程 DSL、自由绘画等图表工具，并提供团队协作、模板市场与后台管理能力。
...
- **任务队列调度**：按并发上限派发、超大文档保护、失败自动重试，SSE 实时推送排版进度
- **排版体检报告**：结构计数与可疑标题提示，一键指定级别重排；排版前后差异对比（差异索引 + 双侧文本定位）
- **校规导入向导**：从校规文档启发式抽取字体/字号/行距/边距/编号规则并附原文依据，逐项确认套用
- **实时预览**：配置页快速试排与实时预览，预览页跟随边距与纸张

### 🔐 账户安全（v1.1.0 新增）
- **双重验证码**：注册/登录需通过图形验证码，注册还须邮箱验证码（QQ 邮箱 SMTP 发送，5 分钟有效，60 秒限流）
- **BCrypt 密码加密**：密码加盐哈希存储，不存明文
- **登录限流**：连续 5 次失败锁定 10 分钟（账号 + IP 双维度），防暴力破解
- **忘记密码**：邮箱验证码重置密码，重置后旧 Token 全部失效
- **账号自管理**：个人资料扩展字段、换绑邮箱与修改密码（双验证码校验），改密后旧 Token 全部吊销
- **XSS 防护**：全局过滤器对 JSON 请求体转义，SQL 注入由 MyBatis-Plus 参数化查询天然防护

### 👥 团队协作
- **团队空间**：创建/邀请（需对方同意）/成员管理/退出/解散
- 模板、任务、论文归属团队并在团队内共享可见，权限越界校验
- 站内信通知：邀请与处理结果、团队模板修订提醒

### 🛒 模板市场
- 模板上架审核、分类筛选与排序、下载量与评分
- 模板收藏、配置导出/导入 JSON、两两参数对比

### 🧭 后台管理
- RBAC 权限体系：菜单/角色/按钮级权限管理
- 用户管理：批量封禁/启用/重置密码；任务重跑与取消；公告定向推送站内信
- 系统参数（上传大小上限/任务并发/文件保留天数）与字典（市场分类）后台可调，运行时生效
- 数据库自动备份（手动/定时/下载）与保留策略，操作/登录审计日志
...
### 🎨 自由绘画
- 内嵌 drawio 官方编辑器：拖拽图形、连线吸附、悬停箭头快捷建图
- 图纸保存/载入、导出 PNG、全部图表支持 Mermaid 按图导出
...
│       ├── controller/         # 接口: 用户/模板/论文/团队/ER图/三线表/系统图
│       ├── service/            # 业务 + 排版引擎 + 图表生成器
│       ├── engine/             # 排版引擎(结构识别/格式化)
│       └── dto/                # 请求/响应模型
├── deploy/                     # Docker 编排/一键部署更新脚本/SQL 迁移
└── frontend/                   # Vue 3 前端
    └── src/
        ├── views/              # 页面(首页/模板/任务/图表工具/团队/后台管理)
        ├── api/                # 接口封装
        └── router/             # 路由
```
...
| `JWT_SECRET` | JWT 密钥，通过环境变量配置 |
| `THESIS_ADMIN_PASSWORD` | 后台管理员密码，通过环境变量配置 |
| `STORAGE_DIR` | 文件存储目录，默认 `./data/storage` |
